...
function problemB () {
  ...
  //NOTA: En este momento se ejecutan mientras se esperan las respuestas de las promesas
}
...
function problemD () {
  ...
  // promise version
  // ???
  //ALTERNATIVA 1 EN DONDE RECIBIMOS EN .THEN DONDE RECIBIMOS 2 PARAMETROS EL POEMA O EL ERROR
  promisifiedReadFile('poem-one/wrong-file-name.txt')
  .then(stanza => blue(stanza), err => magenta(new Error(err)))

  //ALTERNATIVA 2 EN DONDE CAPTAMOS EL ERROR EN .CATCH()
  //Nota 2 en caso de no tener un errorHandle no este definido
  // promisifiedReadFile('poem-one/wrong-file-name.txt')
  // .then(stanza => blue(stanza))
...
function problemF () {
  ...
  .catch(err => magenta(new Error(err)))
  .finally(()=>console.log('done')) // .finally en cualquier caso se ejecuta 
